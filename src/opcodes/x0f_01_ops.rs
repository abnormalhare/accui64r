use crate::{debug::{OpOrder, dbp}, opcodes::std_ops::OpFuncRet, ram::RAM, reg::{ModRM, RegType}, x64::{AddressSize, CPU, OperandSize}};
...
fn lgdt(cpu: &mut CPU, ram: &mut RAM, modrm: &ModRM) {
    ...
    let op_size: OperandSize = cpu.deter_op_size();
    
    match op_size {
        OperandSize::BIT16 => {
            cpu.gdtr.limit = cpu.get_val16(ram, addr);
            cpu.gdtr.base = (cpu.get_val32(ram, addr + 2) & 0x00FFFFFF) as u64;
            
            dbp(cpu, "LGDTW", &new_modrm, 0, 0, OpOrder::RM);
        },
        OperandSize::BIT32 => {
            cpu.gdtr.limit = cpu.get_val16(ram, addr);
            cpu.gdtr.base = cpu.get_val32(ram, addr + 2) as u64;
            
            dbp(cpu, "LGDTD", &new_modrm, 0, 0, OpOrder::RM);
        },
        OperandSize::BIT64 => {
            cpu.gdtr.limit = cpu.get_val16(ram, addr);
            cpu.gdtr.base = cpu.get_val64(ram, addr + 2);
            
            dbp(cpu, "LGDT", &new_modrm, 0, 0, OpOrder::RM);
        }
    }

}

fn op_0f_01_2(cpu: &mut CPU, ram: &mut RAM, modrm: &ModRM) -> bool {
    match modrm._rm {
        0 => {},
        1 => {},
        _ => lgdt(cpu, ram, modrm),
    }
